...
fprintf(1, 'c : toggle coarse/fine scrolling\n');
...
switch lower(keydata.Key)    
    ...
    case 'c' % decrement slice
        ud.coarse = 1 - ud.coarse;
        if ud.coarse
            fprintf('Coarse scrolling activated\n');
        else
            fprintf('Fine scrolling activated\n');
        end
...
factor = ud.coarse * 4 + 1;


% scroll through slices
if ud.scrollMode==0
    ud.currentSlice = ud.currentSlice+evt.VerticalScrollCount*factor;
...
elseif ud.scrollMode==1 %&&  abs(ud.currentAngle(1)) < 130
  ud.currentAngle(1) = ud.currentAngle(1)+evt.VerticalScrollCount*factor;

% scroll through M/L angles
elseif ud.scrollMode==2 %&&  abs(ud.currentAngle(2)) < 130
  ud.currentAngle(2) = ud.currentAngle(2)+evt.VerticalScrollCount*factor; 
end  
